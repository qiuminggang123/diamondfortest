...
export default function LoginModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  if (showRegister) {
    return <RegisterModal open={true} onClose={() => setShowRegister(false)} switchToLogin={() => setShowRegister(false)} />;
  }

  if (showForgot) {
    return <ForgotPasswordModal open={true} onClose={() => setShowForgot(false)} switchToLogin={() => setShowForgot(false)} />;
  }
  ...
  return (
    <form className="flex flex-col gap-4" onSubmit={handleLogin}>
      ...
      <div className="flex justify-between items-center">
        ...
        <button type="button" className="text-blue-500 text-sm underline ml-2 w-1/2 text-right" onClick={setShowForgot} tabIndex={-1}>忘记密码？</button>
      </div>
