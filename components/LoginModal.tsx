...
import { useUIStore } from "@/lib/uiStore"; // Get functions to switch between registration and login
...
export default function LoginModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  if (showForgot) {
    return <ForgotPasswordModal open={true} onClose={() => setShowForgot(false)} onSent={() => setShowForgot(false)} />;
  }
    ...
    <>
      <div className="fixed inset-0 z-200 flex items-center justify-center bg-black/30" onClick={onClose}>
        <div className="bg-white rounded-xl shadow-xl p-8 w-full max-w-sm relative" onClick={e => e.stopPropagation()}>
          ...
          <LoginForm onClose={onClose} setShowForgot={() => setShowForgot(true)} />
          ...
          <div className="mt-4 text-center text-sm">
            ...
            <button 
              ...
              onClick={() => {
                showRegister(true);  // Show registration modal
                onClose();           // Close current login modal
              }}
            >
              Register Now
            </button>
          ...
          <button className="absolute top-2 right-2 text-gray-400 hover:text-gray-700" onClick={onClose} title="Close">×</button>
        </div>
      </div>
    </>
  );
...
function LoginForm({ onClose, setShowForgot }: { onClose: () => void, setShowForgot: () => void }) {
  ...
  return (
    <form className="flex flex-col gap-4" onSubmit={handleLogin}>
      ...
      <div className="flex justify-between items-center">
        <button type="submit" className="bg-blue-600 text-white rounded py-2 font-semibold w-1/2" disabled={loading}>Email Login</button>
        <button type="button" className="text-blue-500 text-sm underline ml-2 w-1/2 text-right" onClick={setShowForgot} tabIndex={-1}>Forgot Password?</button>
      </div>
