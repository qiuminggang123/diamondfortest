...
export default function AdminPage() {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto bg-gray-50 min-h-screen">
      <h1 className="text-3xl font-bold mb-8 text-gray-800">素材库管理后台</h1>
           ...
           <div className="flex flex-wrap gap-3">
               {categories.map(cat => (
                   !['all', 'in-use'].includes(cat.id) && (
                        <div key={cat.id} className={`flex items-center gap-2 px-3 py-2 rounded-lg border ${editingCatId === cat.id ? 'bg-purple-50 border-purple-300 ring-1 ring-purple-300' : 'bg-gray-50 border-gray-200'}`}>
                            <span className="text-sm font-medium text-gray-700">{cat.name}</span>
                            <div className="flex items-center gap-1 ml-2 border-l pl-2 border-gray-300">
                                <button onClick={() => startEditCategory(cat)} className="text-blue-500 hover:bg-blue-50 p-1 rounded"><Edit2 size={12} /></button>
                                <button onClick={() => handleDeleteCategory(cat.id)} className="text-red-500 hover:bg-red-50 p-1 rounded"><Trash2 size={12} /></button>
                            </div>
                        </div>
                   )
               ))}
      ...
      {/* Upload/Edit Section */}
      <div className={`p-6 rounded-2xl shadow-sm mb-8 border-2 transition-colors ${editingId ? 'bg-blue-50 border-blue-200' : 'bg-white border-white'}`}>
        <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-semibold flex items-center gap-2">
                {editingId ? <Edit2 className="w-5 h-5 text-blue-600" /> : <Plus className="w-5 h-5" />} 
                {editingId ? '编辑素材' : '添加新素材'}
            </h2>
            {editingId && (
                <button onClick={resetForm} className="text-sm text-gray-500 hover:text-gray-700 flex items-center gap-1 bg-white px-3 py-1 rounded-full border shadow-sm">
                    <X size={14} /> 取消编辑
                </button>
            )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">珠子名称</label>
                    <input 
                ...
                <div className="grid grid-cols-2 gap-4">
                     <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">类型 (Category)</label>
                        <select 
                            className="w-full px-4 py-2 border rounded-lg"
                            value={newBead.type} 
                            onChange={e => setNewBead({...newBead, type: e.target.value})}
                        >
                            {categories.map(cat => (
                                 !['all', 'in-use'].includes(cat.id) && (
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">尺寸 (mm)</label>
                        <input 
                            type="number" 
                            className="w-full px-4 py-2 border rounded-lg"
                            value={newBead.size} 
                            onChange={e => setNewBead({...newBead, size: Number(e.target.value)})} 
                        />
                    </div>
                </div>
                 ...
                 <div>
                    ...
                    <input 
                        ...
                        className="w-full px-4 py-2 border rounded-lg"
                        value={newBead.price} 
                        onChange={e => setNewBead({...newBead, price: Number(e.target.value)})} 
                    />
                </div>
            ...
            <div className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-gray-300 rounded-xl bg-gray-50/50 relative">
                {newBead.image ? (
                     <div className="relative w-48 h-48 mb-4 group">
                        <img src={newBead.image} alt="Preview" className="w-full h-full object-contain drop-shadow-lg" />
                        <button 
                ...
                <p className="mt-2 text-xs text-gray-400">注意：贴图中必须包含横向水平的绳子穿过珠子中心</p>
            </div>
        </div>
      </div>
